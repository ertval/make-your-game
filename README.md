...
### Narrow Troubleshooting Command Reruns

- If **`npm run policy`** fails, you can isolate it with:
  - **`npm run policy:quality`**: Biome, tests, coverage, schema validation, and SBOM via the project quality gate.
  - **`npm run policy:checks:local`**: Local rerun for ticket association and ownership checks with metadata preparation.
  - **`npm run policy:forbidden`**: Forbidden tech in changed files only.
  - **`npm run policy:header`**: Source headers in changed files only.
  - **`npm run policy:approve`**: Human approval/review requirement.
  - **`npm run policy:repo`**: Repo-wide scans and traceability/dependency pairing.
- If **`npm run policy:repo`** fails, you can isolate it with:
  - **`npm run policy:forbiddenrepo`**: Forbidden tech across the repository.
  - **`npm run policy:headerrepo`**: Source headers across the repository.
  - **`npm run policy:trace`**: Requirement/audit matrix coverage and dependency pairing.
