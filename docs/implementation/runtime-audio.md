...
| Producing the actual `.mp3` audio files; loudness normalization across categories | **C-08** | Draft candidates shipped on `chbaikas/integration-C-08`: 11 SFX + 1 loop-safe music track under `assets/generated/{sfx,music}/`, all registered in `audio-manifest.json` and audible end-to-end via the C-06 adapter + C-07 runner. The shipped pipeline is **MP3-only** (the C-10 manifest gate accepts only `format: "mp3"`); multi-format/open-variant audio is out of scope until the schema and runtime decoder are expanded together. Pending: remaining SFX set, loudness-normalization sign-off, `A-13` gate. |
| Lazy/streaming load policy for non-critical audio; Performance API thresholds for audio decode timing | **C-09** | Pending. |
| `assets/manifests/audio-manifest.json` + JSON Schema validation gate | **C-10** | Implementation complete on `chbaikas/integration-C-10` (PR #219). Strict JSON Schema 2020-12 at `docs/schemas/audio-manifest.schema.json` (required fields, category enum, `additionalProperties: false`, **MP3-only** `format`/path), wired through `npm run validate:schema` (also `npm run ci`). The validator (`scripts/validate-schema.mjs`) also enforces on-disk file existence, kebab-case naming, `maxBytes` budgets, and a fail-closed `DUPLICATE_ID` semantic gate. All shipped C-07/C-08 cue IDs validate cleanly. Verification: `tests/integration/gameplay/c-10-audio-manifest-schema.test.js`. Ticket stays in-progress `[-]` (its `Depends On` C-08 + A-13 are still open). |
