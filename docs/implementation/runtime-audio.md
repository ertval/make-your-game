...
| Producing the actual `.mp3` / `.ogg` audio files; loudness normalization across categories | **C-08** | Draft candidates shipped on `chbaikas/integration-C-08`: 11 SFX + 1 loop-safe music track under `assets/generated/{sfx,music}/`, all registered in `audio-manifest.json` and audible end-to-end via the C-06 adapter + C-07 runner. Pending: remaining SFX set, loudness-normalization sign-off, optional `.ogg`, `A-13` gate. |
| Lazy/streaming load policy for non-critical audio; Performance API thresholds for audio decode timing | **C-09** | Pending. |
| `assets/manifests/audio-manifest.json` + JSON Schema validation gate | **C-10** | JSON Schema at `docs/schemas/audio-manifest.schema.json` is in place; gate is wired through `npm run validate:schema`. Manifest entries for the shipped C-07 cue IDs are now populated alongside the C-08 asset files (draft) and validate cleanly. |
