...
- [-] **C-05** P2 - HUD Adapter & Screen Overlays — ADAPTER SCOPE COMPLETE / READY_FOR_MAIN: NO. Delivered `hud-adapter`, `screens-adapter`, and `storage-adapter` with safe DOM sinks, class-based overlays, keyboard navigation, focus restore, and validated high-score localStorage reads. Verification added in `tests/integration/adapters/*` and `tests/e2e/c-05-screens-navigation.spec.js`. Runtime mounting, bootstrap wiring, and product-flow integration remain pending (Depends on: D-05, C-02, C-04, A-11 audit gate, non-blocking) | Blocks: A-05; A-06; A-08; D-11; A-12
