...
| AUDIT-B-05 | Is code using asynchronicity for performance? | REQ-01 | Semi-Automatable | C-06 ✅ (adapter), C-07 ✅ (cue runner), C-09 ◐ (preload infra), C-11 ✅ (settings persistence), A-09 | `src/adapters/io/audio-adapter.js` runs `fetch → arrayBuffer → decodeAudioData` off the main thread and caches `AudioBuffer`s in internal Maps for instant playback; `tests/integration/adapters/audio-adapter.test.js` validates the async decode path, buffer caching, missing-clip resilience, and visibility lifecycle. `src/adapters/io/audio-integration.js` ships the C-07 cue runner that drains the D-01 event queue each tick and dispatches mapped cues without blocking the simulation phase; `tests/integration/adapters/audio-integration.test.js` validates queue-order playback, overlapping cue dispatch, and music-state debounce across all `GAME_STATE` values. C-09 adds `preloadAudioAssets(cueIds, options)` to the adapter — async, parallel pre-decode of gameplay-critical SFX into the buffer cache with cache reuse, in-flight dedup, and failure tolerance (music/ambience excluded) — plus `preloadWithIndicator` and the `audio-loading-indicator` DOM adapter (flicker-free loading indicator only past the 200ms threshold), and `preloadAudioAssets` returns real `performance.now()` fetch/decode/total timings + cache-hit/miss/fail counts surfaced via `getPreloadStats()`; covered by `tests/integration/adapters/audio-adapter.test.js` (preload + instrumentation suites), `tests/integration/adapters/audio-loading-indicator.test.js`, and `tests/e2e/c-09-audio-loading-indicator.spec.js`. C-11 adds `applyAudioSettings` (volume-only, idempotent) and persisted settings restored via `getAudioSettings` before the first frame — both synchronous lightweight operations that do not touch the audio decode path. Runtime wiring (Track A integration handoff PR) registers the C-06 adapter at `world.resources.audio` and the C-07 runner as a `render`-phase system wrapper. Evidence: [AUDIT-B-05.preload-timing.md](../audit-reports/evidence/AUDIT-B-05.preload-timing.md) documents the async preload timing/cache summary and non-blocking proof. | Mapped, Planned, Executable |
