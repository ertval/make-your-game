...
- Manual evidence obligations for `AUDIT-F-19`, `AUDIT-F-20`, `AUDIT-F-21`, and `AUDIT-B-06` are tracked through `docs/audit-reports/manual-evidence.manifest.json` (all 4 entries signed off by ekaramet 2026-06-23).
...
| REQ-03 | Pause menu contains Continue and Restart | C-04, C-05, A-06 | AUDIT-F-07, AUDIT-F-08, AUDIT-F-09 | `tests/e2e/c-05-screens-navigation.spec.js` + `tests/integration/adapters/screens-adapter.test.js` + `tests/e2e/audit/audit.browser.spec.js` (`pause-controls-contract`, `pause-resume-transition`, `pause-restart-transition`) | Mapped, Covered, Executable (`C-04` owns the pause FSM systems; `C-05` mounts the Continue/Restart overlay in the live runtime shell; browser runtime checks exercise pause → continue → restart end to end.) |
| REQ-04 | HUD shows countdown/timer | C-02, C-05, A-06 | AUDIT-F-14 | `tests/unit/systems/timer-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` + `tests/e2e/audit/audit.browser.spec.js` (`hud-contract`: timer decrements over 60+ live frames) | Mapped, Covered, Executable (`C-02` owns timer logic; `C-05` adapter-level formatting; browser runtime check asserts a real countdown tick in the mounted HUD.) |
...
| REQ-06 | HUD shows lives and lives decrement | C-02, C-05, A-06 | AUDIT-F-16 | `tests/unit/systems/life-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` + `tests/e2e/audit/audit.browser.spec.js` (`hud-contract`: lives value rendered at runtime) | Mapped, Covered, Executable (`C-02` owns life-loss logic incl. GAME_OVER transition; `C-05` adapter-level presentation; browser runtime check asserts the mounted lives HUD.) |
...
| REQ-13 | Single-player only | B-02, C-04, A-06 | AUDIT-F-03 | `tests/e2e/audit/audit.browser.spec.js` | Mapped, Planned, Executable |
| REQ-14 | Genre aligns with pre-approved list | B-03, B-06, B-07, B-08, D-10 | AUDIT-F-06, AUDIT-F-13 | `tests/e2e/audit/audit.browser.spec.js` | Mapped, Planned, Executable |
...
| REQ-16 | C-04 pause and level progression ECS system-layer contracts only | C-04 | AUDIT-F-07, AUDIT-F-08, AUDIT-F-09, AUDIT-F-10 | `REQ-03 -> src/ecs/systems/pause-system.js + tests/unit/systems/pause-system.test.js`; `REQ-09 -> src/ecs/systems/pause-input-system.js + tests/unit/systems/pause-input.test.js`; `REQ-16 -> src/ecs/systems/level-progress-system.js + tests/unit/systems/level-progress-system.test.js`; runtime wiring covered by `tests/e2e/audit/audit.browser.spec.js` + `tests/e2e/game-loop.pause.spec.js` | Mapped, Covered, Executable (system-layer contracts plus full product/runtime wiring via the browser audit suite) |
...
|---|---|---|---|---|---|---|
| AUDIT-F-01 | Does the game run without crashing? | REQ-01, REQ-14 | Fully Automatable | A-06, B-06 | `tests/e2e/audit/audit.browser.spec.js` + `tests/e2e/audit/audit.e2e.test.js` | Mapped, Planned, Executable |
...
| AUDIT-F-03 | Is the game single player? | REQ-13 | Fully Automatable | B-02, C-04 | `tests/e2e/audit/audit.browser.spec.js` | Mapped, Planned, Executable |
| AUDIT-F-04 | Does the game avoid the use of canvas? | REQ-11 | Fully Automatable | A-01, D-06 | `tests/e2e/audit/audit.browser.spec.js` | Mapped, Planned, Executable |
| AUDIT-F-05 | Does the game avoid the use of frameworks? | REQ-12 | Fully Automatable | A-01 | `tests/e2e/audit/audit.e2e.test.js` + static scan dependency check | Mapped, Planned, Executable |
| AUDIT-F-06 | Is the game chosen from the pre-approved list? | REQ-14 | Fully Automatable | B-03, B-06, B-08 | `tests/e2e/audit/audit.browser.spec.js` | Mapped, Planned, Executable |
...
| AUDIT-F-13 | Does game behave like pre-approved genre, including deterministic ghost-house stagger/respawn timing from `game-description.md` §5.4? | REQ-14, REQ-15 | Fully Automatable | B-03, B-06, B-07, B-08, C-03 | `tests/e2e/audit/audit.browser.spec.js` + `src/ecs/systems/spawn-system.js` + `tests/unit/systems/spawn-system.test.js` | Mapped, Covered, Executable (C-03 covers stagger timing, FIFO ordering, cap enforcement, and respawn delay at the system-test level) |
...
| AUDIT-F-17 | Can you confirm there are no frame drops? | REQ-01, REQ-09 | Semi-Automatable | A-06, D-08, A-09 | `tests/e2e/audit/audit.browser.spec.js` + [AUDIT-F-17-F-18.performance.md](../audit-reports/evidence/AUDIT-F-17-F-18.performance.md) | Mapped, Planned, Executable |
| AUDIT-F-18 | Does game run around 60 FPS? | REQ-01 | Semi-Automatable | A-06, D-08, A-09 | `tests/e2e/audit/audit.browser.spec.js` + [AUDIT-F-17-F-18.performance.md](../audit-reports/evidence/AUDIT-F-17-F-18.performance.md) | Mapped, Planned, Executable |
...
| ID | Audit Question | Requirement IDs | Execution Type | Owning Tickets (`docs/implementation/track-*.md`) | Test/Evidence Anchor | Status |
|---|---|---|---|---|---|---|
| AUDIT-B-01 | Does project run quickly and effectively? | REQ-01 | Fully Automatable | A-06, D-08, A-09 | `tests/e2e/audit/audit.browser.spec.js` + [AUDIT-F-17-F-18.performance.md](../audit-reports/evidence/AUDIT-F-17-F-18.performance.md) | Mapped, Planned, Executable |
| AUDIT-B-02 | Does code obey good practices? | REQ-12 | Fully Automatable | A-01, A-07, A-09, C-10 | CI policy gate + `npm run validate:schema` + [AUDIT-B-01-B-04.quality.md](../audit-reports/evidence/AUDIT-B-01-B-04.quality.md) | Mapped, Planned, Executable |
| AUDIT-B-02 (C-10 audio manifest contract) | Does the audio manifest obey the validated MP3-only contract? | REQ-12 | Fully Automatable | C-10 (gate: A-07) | `docs/schemas/audio-manifest.schema.json` (strict 2020-12, MP3-only, `additionalProperties: false`) enforced via `npm run validate:schema` (`scripts/validate-schema.mjs`: schema shape + file existence + kebab-case naming + `maxBytes` + fail-closed `DUPLICATE_ID`); `tests/integration/gameplay/c-10-audio-manifest-schema.test.js` drives the real validator as a subprocess (real manifest passes; missing required field, invalid category, non-mp3 format, out-of-tree path, non-positive `durationMs`, non-boolean `critical`, unknown field, duplicate `id`, and missing asset file all fail closed) | Mapped, Covered, Executable |
| AUDIT-B-03 | Does program reuse memory to avoid jank? | REQ-01 | Fully Automatable | A-02, B-06, D-09, D-08 | `tests/integration/adapters/sprite-pool-adapter.test.js` + [AUDIT-B-01-B-04.quality.md](../audit-reports/evidence/AUDIT-B-01-B-04.quality.md) | Mapped, Covered, Executable |
| AUDIT-B-04 | Does game use SVG? | REQ-14 | Fully Automatable | D-09, D-11 | Static SVG scan + [AUDIT-B-01-B-04.quality.md](../audit-reports/evidence/AUDIT-B-01-B-04.quality.md) | Mapped, Planned, Executable |
| AUDIT-B-05 | Is code using asynchronicity for performance? | REQ-01 | Semi-Automatable | C-06 ✅ (adapter), C-07 ✅ (cue runner), C-11 ✅ (settings persistence), C-09 ◐ (preload infra), A-09 | `src/adapters/io/audio-adapter.js` runs `fetch → arrayBuffer → decodeAudioData` off the main thread and caches `AudioBuffer`s in internal Maps for instant playback; `tests/integration/adapters/audio-adapter.test.js` validates the async decode path, buffer caching, missing-clip resilience, and visibility lifecycle. `src/adapters/io/audio-integration.js` ships the C-07 cue runner that drains the D-01 event queue each tick and dispatches mapped cues without blocking the simulation phase; `tests/integration/adapters/audio-integration.test.js` validates queue-order playback, overlapping cue dispatch, and music-state debounce across all `GAME_STATE` values. C-11 adds `applyAudioSettings` (volume-only, idempotent) and persisted settings restored via `getAudioSettings` before the first frame — both are synchronous lightweight operations that do not touch the audio decode path. C-09 adds `preloadAudioAssets(cueIds, options)` to the adapter — async, parallel pre-decode of gameplay-critical SFX into the buffer cache with cache reuse, in-flight dedup, and failure tolerance (music/ambience excluded) — plus `preloadWithIndicator` and the `audio-loading-indicator` DOM adapter, which surface a flicker-free loading indicator only when the preload crosses the 200ms threshold and hide it on completion; covered by `tests/integration/adapters/audio-adapter.test.js` (preload suite), `tests/integration/adapters/audio-loading-indicator.test.js`, and `tests/e2e/c-09-audio-loading-indicator.spec.js`. Runtime wiring (Track A integration handoff PR) registers the C-06 adapter at `world.resources.audio` and the C-07 runner as a `render`-phase system wrapper. A performance-timing evidence artifact + Playwright `page.evaluate()` Performance API threshold checks remain the final gate (covered by remaining C-09/A-09 scope). | Mapped, Planned, Executable |
| AUDIT-B-06 | Is project well done overall? | REQ-01 through REQ-16 | Manual-With-Evidence | All tracks, A-09, C-08 (audio production), C-11 (audio UX) | [AUDIT-B-06.overall.md](../audit-reports/evidence/AUDIT-B-06.overall.md) + [playthrough-report.md](../audit-reports/evidence/playthrough-report.md) + [playwright-trace.zip](../audit-reports/evidence/playwright-trace.zip) + audio production quality evidence (C-08 candidate SFX/music in `assets/generated/{sfx,music}/` + `assets/manifests/audio-manifest.json`, audible end-to-end via C-06/C-07). C-11 contribution: persistent music/SFX mute toggles and volume controls survive page reload (`localStorage`), bomb-place SFX plays sequentially before the fuse loop (no overlap), and a Settings overlay + always-visible quick-toggle give players control without interrupting gameplay. | Mapped, Covered, Executable (audio candidates shipped in `assets/generated/{sfx,music}/`; C-08/C-10 manifest schema-validated; audio signed off end-to-end via the 2026-06-23 manual-evidence manifest PASS) |
...
| Broadened var check (SEC-02) | #269 | `tests/unit/policy-gate/policy-utils.test.js` | Prevents the use of `var` declarations in source files including statement-chained, inline, and parenthesis-preceded declarations. |
| Chunked map size check (SEC-03-chunked) | #270 | `tests/unit/main.ecs.test.js` | Rejects oversized map assets under chunked-transfer (no `Content-Length` header) using streamed validation. |
| Early unhandled rejection handler (SEC-04) | #271 | `tests/unit/main.ecs.test.js` | Installs the unhandledrejection handler before fetching maps during bootstrap to ensure early errors are caught and rendered. |
