...
| REQ-03 | Pause menu contains Continue and Restart | C-04, C-05, A-06 | AUDIT-F-07, AUDIT-F-08, AUDIT-F-09 | `tests/e2e/c-05-screens-navigation.spec.js` + `tests/integration/adapters/screens-adapter.test.js` | Mapped, Covered, Executable for C-05 overlay and keyboard flow. Full product-level runtime wiring remains shared with later integration tickets. |
| REQ-04 | HUD shows countdown/timer | C-02, C-05, A-06 | AUDIT-F-14 | `tests/unit/systems/timer-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` | Mapped, Covered, Executable (`C-02` owns timer logic; `C-05` now covers HUD formatting/presentation contract.) |
| REQ-05 | HUD shows score and score increments | C-01, C-05, A-06 | AUDIT-F-15 | `tests/unit/systems/scoring-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` | Mapped, Covered, Executable (`C-01` owns scoring logic; `C-05` covers visible score formatting/presence contract.) |
| REQ-06 | HUD shows lives and lives decrement | C-02, C-05, A-06 | AUDIT-F-16 | `tests/unit/systems/life-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` | Mapped, Covered, Executable (`C-02` owns life logic; `C-05` now covers HUD lives presentation.) |
...
| AUDIT-F-07 | Does pause menu show continue and restart? | REQ-03, REQ-09, REQ-16 | Fully Automatable | C-04, C-05 | `tests/integration/adapters/screens-adapter.test.js` + `tests/e2e/c-05-screens-navigation.spec.js` + `tests/unit/systems/pause-input.test.js` + `tests/unit/systems/pause-system.test.js` | Covered, Executable for C-05 overlay/UI contract plus C-04 system-layer pause resources. |
...
| AUDIT-F-14 | Does timer/countdown work? | REQ-04 | Fully Automatable | C-02, C-05 | `tests/unit/systems/timer-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` | Covered, Executable (`C-02` validates countdown logic; `C-05` validates M:SS HUD rendering.) |
| AUDIT-F-15 | Does the score HUD remain present during gameplay, with runtime-visible score increments deferred to later integration? | REQ-05 | Fully Automatable | C-01, C-05, A-06 | `tests/unit/systems/scoring-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` | Covered, Executable (`C-01` validates deterministic score logic; `C-05` validates padded HUD score presentation.) |
| AUDIT-F-16 | Do lives decrease on life-loss events? | REQ-06 | Fully Automatable | C-02, C-05 | `tests/unit/systems/life-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` | Covered, Executable (`C-02` validates life-loss logic; `C-05` validates lives HUD presentation.) |
