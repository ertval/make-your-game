...
| REQ-05 | HUD shows score and score increments | C-01, C-05, A-06 | AUDIT-F-15 | `tests/unit/systems/scoring-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` | Mapped, PARTIAL (`C-01` owns scoring logic; `C-05` currently proves adapter-level score presentation only.) |
...
| AUDIT-F-07 | Does pause menu show continue and restart? | REQ-03, REQ-09, REQ-16 | Fully Automatable | C-04, C-05 | `tests/integration/adapters/screens-adapter.test.js` + `tests/e2e/c-05-screens-navigation.spec.js` + `tests/unit/systems/pause-input.test.js` + `tests/unit/systems/pause-system.test.js` | PARTIAL - adapter-level and system-level only. Full runtime-mounted pause menu behavior remains deferred. |
...
| AUDIT-F-09 | Does restart reset correctly from pause? | REQ-03, REQ-09, REQ-16 | Fully Automatable | C-04, A-05 | `src/ecs/systems/pause-system.js` + `tests/unit/systems/pause-system.test.js` | PARTIAL - C-04 covers restart intent and ECS state contracts only. Full visible restart/reset UX is deferred to later tickets. |
...
| AUDIT-F-14 | Does timer/countdown work? | REQ-04 | Fully Automatable | C-02, C-05 | `tests/unit/systems/timer-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` | PARTIAL - `C-02` validates countdown logic and `C-05` validates adapter-level M:SS HUD rendering only. Runtime-mounted HUD remains deferred. |
| AUDIT-F-15 | Does the score HUD remain present during gameplay, with runtime-visible score increments deferred to later integration? | REQ-05 | Fully Automatable | C-01, C-05, A-06 | `tests/unit/systems/scoring-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` | PARTIAL - `C-01` validates deterministic score logic and `C-05` validates adapter-level padded score presentation only. Runtime-mounted score HUD remains deferred. |
| AUDIT-F-16 | Do lives decrease on life-loss events? | REQ-06 | Fully Automatable | C-02, C-05 | `tests/unit/systems/life-system.test.js` + `tests/integration/adapters/hud-adapter.test.js` | PARTIAL - `C-02` validates life-loss logic and `C-05` validates adapter-level lives HUD presentation only. Runtime-mounted HUD remains deferred. |
