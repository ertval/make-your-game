...
- **F-01 / F-17-18** (runs without crashing / no dropped frames) — #263 removes a silent sprite-drop failure mode under high entity load; net safer. Verified by the new capacity unit test.
- Otherwise pure cleanup with no audit-coverage change.
...
- #263 enlarges the render-intent buffer (425 → 550 slots), a small one-time allocation; it can only *prevent* drops, never cause them. Full suite green (1296).
